...
const EnergyAnalysis = () => {
  ...
  const section2 = [
    {
      header: 'Turning Data Into Insights',
      description:
  ...
  const section5 = [
    {
      ...
      description: 'Data can get too granular for effective analysis and visualization.',
    },
  ];
  ...
  return (
    <Container className="m-10">
      ...
      <Box className="mb-8 p-8 pt-2 bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-300/30 shadow-lg">
        ...
        <Box className="mt-12">{section2 && <MainSection2 sectionProps={section2} />}</Box>
        <br></br>
        Rather than inundating you with every detail, I'll walk give a rough tour of how I
        constructed the results:
        {/* <Box className="mt-12">{section3 && <MainSection2 sectionProps={section3} />}</Box> */}
        <Box className="mt-12">{section4 && <MainSection2 sectionProps={section4} />}</Box>
        Unfortunately, I only learn once I created the data visualizations that the data was too
        granular to really make use of. The chart looked pretty, but I had ran into same trouble I
        encountered with my utility provider's dashboards: the variables measured within the covered
        timeframe was too short to make any useful conclusions.
        <Box className="mt-12">{section5 && <MainSection2 sectionProps={section5} />}</Box>
      </Box>
    </Container>
  );
};
